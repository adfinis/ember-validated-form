...
moduleForComponent(
  "validated-form",
  "Integration | Component | validated form",
  {
    integration: true
  }
);
...
test("it performs basic validations on submit", function(assert) {
  ...
  this.render(hbs`
    ...
    {{/validated-form}}
  `);
  assert.equal(this.$("span.form-text.text-danger").length, 0);
  this.$("button").click();
  assert.equal(this.$("input").val(), "x");
  assert.equal(this.$("span.form-text.text-danger").length, 1);
  assert.equal(
    this.$("span.form-text.text-danger").text(),
